...
// const _rest_url = `${langApi}/interviews?scene=${_id}&context=embed`
let _params = ref(`interview_category=${_cat_id}&per_page=${_perPage}&page=${_current}&context=embed`)
...
onMounted(() => {
  stClass.value = {type:"archive",cls:"interviews scene tax",lng:locale.value}
  // console.log(_posts)
  fetchData();
})
</script>
<template>
<section class="sec_interviews sec">
    <div class="sec__inner">
      <div class="page__header">
        <h2 class="sec-ttl"><NuxtLinkLocale :to="`/interviews`">{{ t('interview') }}</NuxtLinkLocale> / <span v-html="catName"></span></h2>
        <Select type="interviews" taxtype="interview_category" :catid="_cat_id" v-model="catName" />
      </div>
      <div v-if="_posts">
    ...
    <Pagination :numPages="_numPage" :current="_current" :total="_limit" :urlSlug="`${_postType}/category/${_cat_id}`" />
  </section>
</template>
